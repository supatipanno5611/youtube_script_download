...
class RegexReplaceModal extends Modal {
    ...
    // 플래그를 각각의 boolean 상태로 관리 (기본값: 전역 검색만 켬)
    flagG: boolean = true;
    flagI: boolean = false;
    flagM: boolean = false;
    ...
    // 활성화된 플래그들을 모아서 하나의 문자열로 반환하는 헬퍼 메서드
    getFlags(): string {
        let flags = '';
    ...
    onOpen() {
        ...
        new Setting(contentEl)
            .setName('Flags')
            .setDesc('g (전역), i (대소문자 무시), m (여러 줄)')
            .addToggle(toggle => toggle
                .setTooltip('Global (g) - 문서 전체에서 찾기')
                .setValue(this.flagG)
                .onChange(value => this.flagG = value))
            .addToggle(toggle => toggle
                .setTooltip('Case Insensitive (i) - 대소문자 무시')
                .setValue(this.flagI)
                .onChange(value => this.flagI = value))
            .addToggle(toggle => toggle
                .setTooltip('Multiline (m) - 여러 줄 모드')
                .setValue(this.flagM)
                .onChange(value => this.flagM = value));
    ...
    handlePreview() {
        ...
        try {
            const regex = new RegExp(this.findStr, this.getFlags());
            ...
            // 해당 행에만 가상으로 치환 적용
            const localRegex = new RegExp(this.findStr, this.getFlags());
